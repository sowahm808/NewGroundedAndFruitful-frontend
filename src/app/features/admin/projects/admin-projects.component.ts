import {
  ChangeDetectionStrategy,
  Component,
  OnInit,
  inject,
  signal,
} from '@angular/core';
import { CommonModule } from '@angular/common';
import {
  FormBuilder,
  FormGroup,
  ReactiveFormsModule,
  Validators,
} from '@angular/forms';
import { HttpClient, HttpParams } from '@angular/common/http';
import {
  GfAlert,
  GfBadge,
  GfEmptyState,
  GfPageHeader,
} from '../../../shared/components/design-system';

export interface ProjectItem {
  id: string;
  title: string;
  category: 'service' | 'creative' | 'learning' | 'faith' | 'community';
  targetWeek: number;
  milestoneCount: number;
  points: number;
  guidancePrompt?: string;
  status: 'draft' | 'published' | 'archived';
  version: number;
  updatedAt: string;
}

@Component({
  selector: 'gf-admin-projects',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    GfAlert,
    GfBadge,
    GfEmptyState,
    GfPageHeader,
  ],
  template: `
    <gf-page-header title="Projects" eyebrow="Administration">
      <p>Oversee and configure personal project tracks, milestones, and guidance scopes.</p>
    </gf-page-header>

    <div class="projects-container" style="max-width: 1200px; margin: 0 auto; padding: 1rem 0;">
      <!-- Filter Bar & Create Trigger -->
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;">
        <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
          <input
            type="text"
            placeholder="Search project tracks..."
            [value]="searchTerm()"
            (input)="onSearchInput($event)"
            style="padding: 0.5rem 0.75rem; border: 1px solid #ccc; border-radius: 6px; min-width: 220px;"
          />
          <select
            [value]="selectedStatus()"
            (change)="onStatusChange($event)"
            style="padding: 0.5rem 0.75rem; border: 1px solid #ccc; border-radius: 6px;"
          >
            <option value="">All statuses</option>
            <option value="draft">Draft</option>
            <option value="published">Published</option>
            <option value="archived">Archived</option>
          </select>
        </div>

        <button
          type="button"
          class="gf-button gf-button--primary"
          style="padding: 0.6rem 1.2rem; cursor: pointer; border-radius: 6px; background: #1b4d3e; color: #fff; border: none; font-weight: 600;"
          (click)="openModal()"
        >
          + Create Project Track
        </button>
      </div>

      <!-- Create Project Track Modal -->
      @if (showModal()) {
        <div
          class="modal-backdrop"
          role="dialog"
          aria-modal="true"
          style="position: fixed; inset: 0; background: rgba(0, 0, 0, 0.45); display: flex; align-items: center; justify-content: center; z-index: 1000;"
        >
          <div
            class="modal-card"
            style="background: #ffffff; padding: 2rem; border-radius: 8px; width: 100%; max-width: 520px; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.15);"
          >
            <h3 style="margin-top: 0; font-size: 1.25rem; font-weight: 700;">Create Project Track</h3>

            @if (errorMessage()) {
              <gf-alert title="Error">
                <p>{{ errorMessage() }}</p>
              </gf-alert>
            }

            <form [formGroup]="projectForm" (ngSubmit)="onCreateProject()">
              <div style="margin-bottom: 1rem;">
                <label for="title" style="display: block; margin-bottom: 0.25rem; font-weight: 600; font-size: 0.875rem;">
                  Project Track Title
                </label>
                <input
                  id="title"
                  type="text"
                  formControlName="title"
                  placeholder="e.g. Community Service Outreach"
                  style="width: 100%; box-sizing: border-box; padding: 0.6rem; border: 1px solid #ccc; border-radius: 4px;"
                />
              </div>

              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                <div>
                  <label for="category" style="display: block; margin-bottom: 0.25rem; font-weight: 600; font-size: 0.875rem;">
                    Category
                  </label>
                  <select
                    id="category"
                    formControlName="category"
                    style="width: 100%; box-sizing: border-box; padding: 0.6rem; border: 1px solid #ccc; border-radius: 4px;"
                  >
                    <option value="service">Service & Outreach</option>
                    <option value="creative">Creative & Expression</option>
                    <option value="learning">Skills & Learning</option>
                    <option value="faith">Faith & Ministry</option>
                    <option value="community">Community Building</option>
                  </select>
                </div>
                <div>
                  <label for="targetWeek" style="display: block; margin-bottom: 0.25rem; font-weight: 600; font-size: 0.875rem;">
                    Target Week (1-12)
                  </label>
                  <input
                    id="targetWeek"
                    type="number"
                    formControlName="targetWeek"
                    style="width: 100%; box-sizing: border-box; padding: 0.6rem; border: 1px solid #ccc; border-radius: 4px;"
                  />
                </div>
              </div>

              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                <div>
                  <label for="milestoneCount" style="display: block; margin-bottom: 0.25rem; font-weight: 600; font-size: 0.875rem;">
                    Milestones (1-5)
                  </label>
                  <input
                    id="milestoneCount"
                    type="number"
                    formControlName="milestoneCount"
                    style="width: 100%; box-sizing: border-box; padding: 0.6rem; border: 1px solid #ccc; border-radius: 4px;"
                  />
                </div>
                <div>
                  <label for="points" style="display: block; margin-bottom: 0.25rem; font-weight: 600; font-size: 0.875rem;">
                    Total Points
                  </label>
                  <input
                    id="points"
                    type="number"
                    formControlName="points"
                    style="width: 100%; box-sizing: border-box; padding: 0.6rem; border: 1px solid #ccc; border-radius: 4px;"
                  />
                </div>
              </div>

              <div style="margin-bottom: 1.5rem;">
                <label for="guidancePrompt" style="display: block; margin-bottom: 0.25rem; font-weight: 600; font-size: 0.875rem;">
                  Mentor Guidance & Project Prompt
                </label>
                <textarea
                  id="guidancePrompt"
                  rows="3"
                  formControlName="guidancePrompt"
                  placeholder="Outline expectations, milestones, and mentor review steps..."
                  style="width: 100%; box-sizing: border-box; padding: 0.6rem; border: 1px solid #ccc; border-radius: 4px; resize: vertical;"
                ></textarea>
              </div>

              <div style="display: flex; justify-content: flex-end; gap: 0.75rem;">
                <button
                  type="button"
                  style="padding: 0.6rem 1.2rem; cursor: pointer; border-radius: 4px; border: 1px solid #ccc; background: #fff;"
                  (click)="closeModal()"
                  [disabled]="isSubmitting()"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  style="padding: 0.6rem 1.2rem; cursor: pointer; border-radius: 4px; border: none; background: #1b4d3e; color: #fff; font-weight: 600;"
                  [disabled]="projectForm.invalid || isSubmitting()"
                >
                  {{ isSubmitting() ? 'Saving...' : 'Create Track' }}
                </button>
              </div>
            </form>
          </div>
        </div>
      }

      <!-- Data Table -->
      @if (isLoading()) {
        <div style="padding: 2rem; text-align: center; color: #666;">Loading project tracks...</div>
      } @else if (projects().length === 0) {
        <gf-empty-state
          title="No project tracks found"
          message="Create your first project track using the '+ Create Project Track' button above."
        />
      } @else {
        <div style="overflow-x: auto; background: #fff; border-radius: 8px; border: 1px solid #e0e0e0;">
          <table style="width: 100%; border-collapse: collapse; text-align: left;">
            <thead>
              <tr style="border-bottom: 1px solid #e0e0e0; background: #fafafa; font-size: 0.85rem; color: #666;">
                <th style="padding: 0.75rem 1rem;">Target Week</th>
                <th style="padding: 0.75rem 1rem;">Track Title</th>
                <th style="padding: 0.75rem 1rem;">Category</th>
                <th style="padding: 0.75rem 1rem;">Milestones</th>
                <th style="padding: 0.75rem 1rem;">Total Points</th>
                <th style="padding: 0.75rem 1rem;">Status</th>
                <th style="padding: 0.75rem 1rem; text-align: right;">Actions</th>
              </tr>
            </thead>
            <tbody>
              @for (item of projects(); track item.id) {
                <tr style="border-bottom: 1px solid #eee; font-size: 0.9rem;">
                  <td style="padding: 0.75rem 1rem; font-weight: 600;">Wk {{ item.targetWeek }}</td>
                  <td style="padding: 0.75rem 1rem;">
                    <strong>{{ item.title }}</strong>
                    @if (item.guidancePrompt) {
                      <div style="color: #666; font-size: 0.8rem;">{{ item.guidancePrompt }}</div>
                    }
                  </td>
                  <td style="padding: 0.75rem 1rem; text-transform: capitalize;">{{ item.category }}</td>
                  <td style="padding: 0.75rem 1rem;">{{ item.milestoneCount }} steps</td>
                  <td style="padding: 0.75rem 1rem;">{{ item.points }} pts</td>
                  <td style="padding: 0.75rem 1rem;">
                    <gf-badge>{{ item.status }}</gf-badge>
                  </td>
                  <td style="padding: 0.75rem 1rem; text-align: right;">
                    @if (item.status === 'draft') {
                      <button
                        type="button"
                        style="padding: 0.3rem 0.6rem; cursor: pointer; border: 1px solid #1b4d3e; background: #1b4d3e; color: #fff; border-radius: 4px; font-size: 0.8rem;"
                        (click)="publishProject(item)"
                      >
                        Publish
                      </button>
                    }
                  </td>
                </tr>
              }
            </tbody>
          </table>
        </div>
      }
    </div>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class AdminProjectsComponent implements OnInit {
  private readonly http = inject(HttpClient);
  private readonly fb = inject(FormBuilder);

  readonly projects = signal<ProjectItem[]>([]);
  readonly isLoading = signal<boolean>(true);
  readonly isSubmitting = signal<boolean>(false);
  readonly showModal = signal<boolean>(false);
  readonly errorMessage = signal<string | null>(null);

  readonly searchTerm = signal<string>('');
  readonly selectedStatus = signal<string>('');

  readonly projectForm: FormGroup = this.fb.group({
    title: ['', [Validators.required, Validators.minLength(2), Validators.maxLength(120)]],
    category: ['service', [Validators.required]],
    targetWeek: [1, [Validators.required, Validators.min(1), Validators.max(12)]],
    milestoneCount: [3, [Validators.required, Validators.min(1), Validators.max(5)]],
    points: [100, [Validators.required, Validators.min(10)]],
    guidancePrompt: ['', [Validators.maxLength(1000)]],
  });

  ngOnInit(): void {
    this.fetchProjects();
  }

  openModal(): void {
    this.errorMessage.set(null);
    this.showModal.set(true);
  }

  closeModal(): void {
    this.showModal.set(false);
  }

  onSearchInput(event: Event): void {
    const value = (event.target as HTMLInputElement).value;
    this.searchTerm.set(value);
    this.fetchProjects();
  }

  onStatusChange(event: Event): void {
    const value = (event.target as HTMLSelectElement).value;
    this.selectedStatus.set(value);
    this.fetchProjects();
  }

  fetchProjects(): void {
    this.isLoading.set(true);
    let params = new HttpParams();
    if (this.selectedStatus()) params = params.set('status', this.selectedStatus());
    if (this.searchTerm()) params = params.set('search', this.searchTerm());

    this.http
      .get<{ data: { items: any[] } }>('/api/v1/admin/projects', { params })
      .subscribe({
        next: (res) => {
          const items = (res.data?.items || []).map((raw) => ({
            id: raw.id,
            title: raw.name || raw.data?.title || 'Untitled Project Track',
            category: raw.data?.category || 'service',
            targetWeek: Number(raw.data?.targetWeek ?? 1),
            milestoneCount: Number(raw.data?.milestoneCount ?? 3),
            points: Number(raw.data?.points ?? 100),
            guidancePrompt: raw.data?.guidancePrompt || '',
            status: raw.status || 'draft',
            version: raw.version || 1,
            updatedAt: raw.updatedAt || new Date().toISOString(),
          }));
          this.projects.set(items);
          this.isLoading.set(false);
        },
        error: (err) => {
          this.projects.set([]);
          this.isLoading.set(false);
          this.errorMessage.set(err.error?.error?.message || 'Failed to load project tracks.');
        },
      });
  }

  onCreateProject(): void {
    if (this.projectForm.invalid) return;

    this.isSubmitting.set(true);
    this.errorMessage.set(null);

    const formVal = this.projectForm.value;
    const payload = {
      name: formVal.title,
      data: {
        title: formVal.title,
        category: formVal.category,
        targetWeek: Number(formVal.targetWeek),
        milestoneCount: Number(formVal.milestoneCount),
        points: Number(formVal.points),
        guidancePrompt: formVal.guidancePrompt,
      },
    };

    this.http.post('/api/v1/admin/projects', payload).subscribe({
      next: () => {
        this.isSubmitting.set(false);
        this.closeModal();
        this.projectForm.reset({ category: 'service', targetWeek: 1, milestoneCount: 3, points: 100 });
        this.fetchProjects();
      },
      error: (err) => {
        this.isSubmitting.set(false);
        this.errorMessage.set(err.error?.error?.message || 'Failed to create project track.');
      },
    });
  }

  publishProject(item: ProjectItem): void {
    this.http
      .post(`/api/v1/admin/projects/${item.id}/publish`, { version: item.version })
      .subscribe({
        next: () => this.fetchProjects(),
        error: (err) => {
          this.errorMessage.set(err.error?.error?.message || 'Failed to publish project track.');
        },
      });
  }
}