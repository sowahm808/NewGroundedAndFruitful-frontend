...
describe('AdminBibleApiService', () => {
  ...
  it('archives a content set with its expected version', () => {
    service.archiveContent('content/1', 8).subscribe();
    const request = http.expectOne((candidate) => candidate.url.endsWith('/admin/bible-content/content%2F1/archive'));
    expect(request.request.method).toBe('POST');
    expect(request.request.body).toEqual({ expectedVersion: 8 });
