...
API base URLs are selected by Angular build configuration. Development uses `http://localhost:3000/api/v1`, staging uses the staging Render host, and production uses `https://api.groundedandfruitful.org/api/v1`. Feature code uses `ApiClient`, whose normalized errors let the UI handle authentication, authorization, not-found, conflict, validation, rate-limit, and server failures consistently.
